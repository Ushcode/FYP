...
def F(x, y):
    ...
    z = y*np.sin(4*x) + 3*np.cos(1.1*x*y+2.3)
    return z > -1.9
y = F(X[:, 0], X[:, 1])  # targets
...
nn.add(Dense(units=1, activation='sigmoid'))  # no activation on output for regression
print(nn.summary())
...
# train
epochs = 100
history = nn.fit(x=X_train, y=y_train, epochs=epochs, batch_size=16, validation_data=(X_val, y_val), callbacks=callbacks)
